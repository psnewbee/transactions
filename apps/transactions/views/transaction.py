...
from extentions import TransactionTypesEnum, exceptions
...
@extend_schema(
    ...
    responses={
        ...
        400: OpenApiResponse(
            description="Error: You do not have category with related ID."
        ),
        401: OpenApiResponse(
...
def create_transaction(request: HttpRequest):
    ...
    if category_id:
        if not UserCategory.objects.filter(id=category_id, user__id=user_id).exists():
            raise exceptions.BadRequest(
                detail="You do not have category with provided ID."
            )
    ...
    return Response(
        data=TransactionInfoSerializer(created_transaction).data,
...
def update_transaction(request: HttpRequest, transaction_id: int):
    ...
    transaction = Transaction.objects.get(
        id=transaction_id, user_transaction_for_transaction__user__id=user_id
    )
    ...
    return Response(
        data=TransactionInfoSerializer(updated_transaction).data,
...
@extend_schema(
    ...
    responses={
        ...
        400: OpenApiResponse(
            description="Error: You do not have transaction with related ID."
        ),
        401: OpenApiResponse(
            description="Error: Authentication credentials were not provided."
        ),
...
def delete_transaction(request: HttpRequest, transaction_id: int):
    try:
        ...
        raise exceptions.BadRequest(
            detail="You do not have transaction with related ID."
        )
